...
export default Component.extend(ThrottledResize, {
  ...
  create() {
    const chart = echarts.init(this.$('.content')[0], 'light');
    ...
    get(this, 'fields').forEach((field) => {
      if ( get(this, `model.${ field }`) ) {
        hasInitData = true;
      }
      this.addObserver(`model.${ field }`, this, 'draw');
    });
  ...
  draw() {
    ...
    get(this, 'fields').forEach((field, index) => {
      const serie = get(this, `model.${ field }`) || [];
      const data = [];
      let dateIndex = 0;
      ...
      tooltip: {
        ...
        formatter(params) {
          ...
          params.forEach((p, i) => {
            ...
            html += `<br><span style="display:inline-block;margin-right:5px;border-radius:10px;width:10px;height:10px;background-color:${ p.color };"></span>${ intl.t(`infoMultiStats.${ p.seriesName }`) }: ${ value }`;
          });
