...
export default Mixin.create({
  ...
  actions: {
    query(showLoading = true){
      ...
      const isIstio = get(this, 'istio');

      if ( get(this, 'projectScope') ) {
        url = `/v3/${ isIstio ? 'istioProjectMonitorGraphs' : 'projectmonitorgraphs' }?action=query`;
      } else {
        url = `/v3/${ isIstio ? 'istioClusterMonitorGraphs' : 'clustermonitorgraphs' }?action=query`;
      }
