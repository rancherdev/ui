...
let Pipeline = Resource.extend({
  ...
  unsyncBranches: computed('unSyncConfigs', function() {

    return Object.keys(get(this, 'unSyncConfigs') || {}).map((branch) => {
  ...
  availableActions: computed('actions.{run,reload}', 'links.config', 'unsyncBranches.length', function() {
    ...
    return [{ divider: true },
      ...
      },
      {
        ...
        action:   'reload',
        enabled:  !!a.reload && get(this, 'unsyncBranches.length') > 0,
        bulkable: false
      },
      { divider: true },
      {
  ...
  actions: {
    reload() {
      ...
      get(this, 'modalService').toggleModal('modal-pipeline-reload', {
        originalModel: this,
        escToClose:    true,
      });
    ...
    viewConfig() {
      ...
      get(this, 'modalService').toggleModal('modal-pipeline-viewconfig', {
        originalModel: this,
        escToClose:    true,
      });
